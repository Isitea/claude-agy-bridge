...
```
payload ≤ 인라이닝 예산  → A (인라이닝)
payload >  인라이닝 예산  → C (서빙)로 자동 전환, 도구 결과에 전환 사실과 사유를 명시
mode ∈ {review, verify} 이고 파일이 여러 개  → 검토 대상은 A, 주변 자료는 C  (혼합)
```

인라이닝 예산은 **문자·바이트 병행**이다 (구현 §2.3-D 보강):
- 문자 상한 = `max_inline_chars` (기본 100,000자)
- 바이트 상한 = `min(max_chars×3, argv 한계 130,000 B − 프롬프트 오버헤드 − 헤드룸)`

바이트 상한을 병행하는 이유: argv 단일 인자 한계는 바이트 기준(§2.3-D)이라, 문자만
세면 멀티바이트 자료(한글 3 B/자)가 문자 예산을 통과하고도 조립 후 한계를 넘어 auto
폴백이 발동하지 못한 채 실패한다. 프롬프트 오버헤드(플레이북/오버레이·mode 지시문·
question·context)도 argv 공간을 나눠 쓰므로 예산에서 함께 차감한다 — 그래서 큰
오버레이나 긴 question은 인라이닝 여력을 줄여 초과분을 서빙으로 민다. 실측: 순한글
문서는 약 40,000자에서 바이트 상한이 먼저 걸린다.
