...
| 항목 | 필수 | 확인 방법 |
|---|---|---|
| `agy` CLI 설치 | 필수 | `agy --version` |
| agy 인증 (OAuth) | 필수 | `agy models` 가 모델 목록을 출력 |
...
| agy 도구 권한 설정 | **불필요** | 인라이닝·루프백 서빙 모두 권한이 필요 없다 |
| agy workspace 신뢰 (trust) | **불필요** | 아래 주석 — 브리지는 중립 스크래치 디렉터리에서 agy를 실행한다 |
| agy 과학 스킬 팩 | 선택 | 아래 "외부 스킬 팩" — 저장소에 벤더링하지 않는다 |

두 필수 요건은 실패 지점이 다르다. **설치**가 없으면 브리지가 기동조차 못 하고
(`agy-bridge doctor` 첫 항목에서 실패), **인증**이 없으면 기동은 되지만 스모크
왕복이 실패한다.

> **workspace 신뢰(trust)는 브리지 사용에 필요하지 않다** — 직관과 어긋나므로
> 적어 둔다. 브리지는 대상 저장소가 아니라 중립 스크래치 디렉터리에서 agy를
> 실행하고(§8.2 — 저장소의 `AGENTS.md`/`GEMINI.md`를 검토자가 상속하지 않게 하기
> 위해서다), 검토 자료는 파일 접근이 아니라 프롬프트 인라이닝과 루프백 URL로
> 전달한다. 그래서 신뢰가 가르는 도구 권한을 애초에 쓰지 않는다.
> 실측(v0.3.2): `trustedWorkspaces`에 없는 새 디렉터리에서 print 모드 왕복과
> 루프백 URL fetch가 모두 성공했고, 신뢰를 묻는 절차도 없었다.
> agy를 브리지 밖에서 직접 쓸 때는 물론 그쪽 요건을 따른다.
